...
from metis_crypto import jwa
from metis_crypto import jwe
from metis_crypto import jwk
from metis_crypto import jws
from metis_crypto import jwt
from metis_crypto.common import InvalidJWSERegOperation
from metis_crypto.common import JWKeyNotFound
from metis_crypto.common import JWSEHeaderParameter
from metis_crypto.common import base64url_decode, base64url_encode
from metis_crypto.common import json_decode, json_encode
...
class TestJWK:
    def test_create_pub_keys(self):
        keylist = PublicKeys['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_pri_keys(self):
        keylist = PrivateKeys['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_sym_keys(self):
    ...
    def test_generate_keys(self):
        ...
        assert k['kid'] == 'MySymmetricKey'
        assert len(base64url_decode(k.get_op_key('encrypt'))) == 24
        jwk.JWK.generate(kty='RSA', alg='RS256')
        k = jwk.JWK.generate(kty='RSA', size=4096, alg='RS256')
        assert k.get_op_key('encrypt').key_size == 4096

    def test_export_public_keys(self):
        k = jwk.JWK(**RSAPrivateKey)
        jk = k.export_public()
        assert not 'd' in json_decode(jk)
    ...
    def test_import_pyca_keys(self):
        ...
        assert krsa1['kty'] == 'RSA'
        krsa2 = jwk.JWK.from_pyca(rsa1.public_key())
        assert krsa1.get_op_key('verify').public_numbers().n == \
    ...
    def test_jwkset_get_keys(self):
        ...
        kwargs['kid'] = '1'
        k2 = jwk.JWK(**kwargs)
        assert k1 == ksm.get_key('1')
        assert ksm.get_key('not-there') is None

        ksm.add(k2)
        assert {k1, k2} == ksm.get_keys('1')
        assert 3 == len(ksm['keys'])
        # Expect that duplicate kids will
        # raise an exception when we use get_key
        with pytest.raises(jwk.InvalidJWKValue):
            ksm.get_key('1')
    ...
    def test_jwkset_issue_208(self):
        ...
        assert len(ks['keys']) == len(ks2['keys'])
    ...
    def test_export_public(self):
        ...
        assert json_decode(pubc)["kid"] == PublicCertThumbprint
    ...
    def test_public(self):
        ...
        assert d.payload == b'plaintext'

    def test_invalid_value(self):
        with pytest.raises(jwk.InvalidJWKValue):
            jwk.JWK(kty='oct', k=b'\x01')

    def test_create_pub_keys_eddsa(self):
        keylist = PublicKeys_EdDsa['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_pri_keys_eddsa(self):
        keylist = PrivateKeys_EdDsa['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_pub_keys_secp256k1(self):
        keylist = PublicKeys_secp256k1['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_pri_keys_secp256k1(self):
        keylist = PrivateKeys_secp256k1['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_pub_keys_brainpool(self):
        keylist = PublicKeys_brainpool['keys']
        for key in keylist:
            jwk.JWK(**key)

    def test_create_pri_keys_brainpool(self):
    ...
    def test_pem_okp(self):
        ...
        assert jws_token.objects['valid']
        assert jws_token.payload == payload
...
class TestJWS:
    ...
    def test_a5(self):
        pytest.raises(jws.InvalidJWSOperation,
    ...
    def test_a6(self):
        ...
        assert A6_example['jose_header'] == s.jose_header
    ...
    def test_customhdr_jws_exists(self):
        ...
        newreg = {'alg': newhdr}
        with pytest.raises(InvalidJWSERegOperation):
            jws.JWS(A6_example['payload'], header_registry=newreg)

    def test_ed_dsa_signing_and_verification(self):
        examples = []
    ...
    def test_secp256k1_signing_and_verification(self):
        ...
        jws_verify.deserialize(jws_test_serialization_compact)
        jws_verify.verify(key.public())
        assert jws_verify.payload == payload
...
class TestJWE:
    ...
    def test_compact_protected_header(self):
        ...
        with pytest.raises(jwe.InvalidJWEOperation):
    ...
    def test_jwe_issue_136(self):
        ...
        enc = e.serialize()
        e.deserialize(enc, key)
        assert e.payload == plaintext.encode('utf-8')
...
class TestMMA:
    ...
    def setup_class(cls):
        import os
        cls.enableMMA = os.environ.get('metis_crypto_TESTS_ENABLE_MMA', False)
        cls.iterations = 500
...
class TestJWT:
    ...
    def test_a1(self):
        ...
        pytest.raises(jwt.JWTExpired, jwt.JWT, jwt=A1_token, key=key,
                          algs=jwe_algs_and_rsa1_5)
    ...
    def test_a2(self):
        ...
        assert A1_claims == json_decode(tinner.claims)
    ...
    def test_decrypt_keyset(self):
        ...
        pytest.raises(jwt.JWTMissingKey, jwt.JWT, jwt=token, key=keyset,
                          algs=jwe_algs_and_rsa1_5,
    ...
    def test_decrypt_keyset_dup_kid(self):
        ...
        # try to decrypt without a matching key
        with pytest.raises(jwt.JWTMissingKey):
            jwt.JWT(jwt=token, key=keyset, algs=jwe_algs_and_rsa1_5,
                    check_claims={'exp': 1300819380})
    ...
    def test_invalid_claim_type(self):
        ...
        # Wrong string
        pytest.raises(jwt.JWTInvalidClaimValue, jwt.JWT, jwt=token,
    ...
    def test_issue_277(self):
        ...
        jwt.JWT(key=key, jwt=sertok, check_claims={"aud": ["www.example.com",
                                                           "nomatch"]})
        pytest.raises(jwt.JWTInvalidClaimValue, jwt.JWT, key=key,
...
class TestConformance:
    ...
    def test_unknown_key_params(self):
        ...
        assert 'mystery' == key.get('unknown')

    def test_key_ops_values(self):
        pytest.raises(jwk.InvalidJWKValue, jwk.JWK,
    ...
    def test_jwe_no_alg_in_jose_headers(self):
        enc = jwe.JWE(plaintext='plain')
        pytest.raises(jwe.InvalidJWEData, enc.add_recipient,
    ...
    def test_jws_headers_as_dicts(self):
        ...
        assert check.objects['valid']
        assert check.jose_header['kid'] == key.thumbprint()
    ...
    def test_jwe_headers_as_dicts(self):
        ...
        check.deserialize(o)
        check.decrypt(key)
        assert check.payload == b'message'
        assert json_decode(check.objects['header'])['kid'] == key.thumbprint()
    ...
    def test_none_key(self):
        ...
        assert json_decode(token.claims) == \
    ...
    def test_no_default_rsa_1_5(self):
        s = jws.JWS('test')
        with pytest.raises(jws.InvalidJWSOperation) as excinfo:
    ...
    def test_pbes2_hs256_aeskw_custom_params(self):
        ...
        key = jwk.JWK.from_password('password')
        pytest.raises(ValueError, enc.add_recipient, key)
...
class TestJWA:
    def test_jwa_create(self):
        for name, cls in jwa.JWA.algorithms_registry.items():
            ...
            assert cls.algorithm_usage_location in {'alg', 'enc'}
            if name == 'ECDH-ES':
                assert cls.keysize is None
            elif name == 'EdDSA':
                assert cls.keysize is None
            else:
                assert isinstance(cls.keysize, int)
                assert cls.keysize >= 0

            if cls.algorithm_use == 'sig':
                with pytest.raises(jwa.InvalidJWAAlgorithm):
                    jwa.JWA.encryption_alg(name)
                with pytest.raises(jwa.InvalidJWAAlgorithm):
                    jwa.JWA.keymgmt_alg(name)
                inst = jwa.JWA.signing_alg(name)
                assert isinstance(inst, jwa.JWAAlgorithm)
...
class TestUnencodedPayload:
    ...
    def test_compat_unencoded(self):
        ...
        sig = s.serialize()
        assert json_decode(sig)['payload'] == rfc7797_payload
    ...
    def test_misses_crit(self):
        s = jws.JWS(rfc7797_payload)
        with pytest.raises(jws.InvalidJWSObject):
            s.add_signature(jwk.JWK(**SymmetricKeys['keys'][1]),
...
class TestOverloadedOperators:
    ...
    def test_jws_representations(self):
        ...
        token.add_signature(key, None,
                            ...
                            json_encode({"kid": key.thumbprint()}))
        ser = token.serialize()
        assert str(token) == ser
        assert repr(token) == f'JWS.from_json_token("{ser}")'
    ...
    def test_jwe_representations(self):
        ...
        assert str(token) == strrep
        assert repr(token) == strrep

        token.add_recipient(key)
        ser = token.serialize()
        assert str(token) == ser
        assert repr(token) == f'JWE.from_json_token("{ser}")'
